...
- **Debugging guest crashes under the ios-linuxkit emulator** (Bun/JSC/Node SIGSEGVs, memory corruption): follow `docs/DEBUGGING_EMULATOR_MEMORY_CRASHES.md` — an agent playbook. Short version: a guest crash at a weird constant address is some *earlier-corrupted value*; make the run **deterministic** (pin `getrandom` + clock), then put an **lldb hardware watchpoint on the corrupted memory's HOST address** to catch the exact writer. If the writer is engine C with `guest_pc=0`, it's a kernel/mm bug, not the JIT. (That's how the long-standing claude-code `0xF7FFDFF8` crash was traced to `sys_madvise` zeroing file-backed pages — DCMMC/ios-linuxkit#6.)
